...
func (cq *CustomerQuery) QueryReserves() *ReserveRoomQuery {
	query := &ReserveRoomQuery{config: cq.config}
...
func (cq *CustomerQuery) QueryFixs() *FixRoomQuery {
	query := &FixRoomQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CustomerQuery) QueryCheckins() *CheckInQuery {
...
func (cq *CustomerQuery) WithReserves(opts ...func(*ReserveRoomQuery)) *CustomerQuery {
	query := &ReserveRoomQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withReserves = query
	return cq
}
...
func (cq *CustomerQuery) WithFixs(opts ...func(*FixRoomQuery)) *CustomerQuery {
	query := &FixRoomQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) WithCheckins(opts ...func(*CheckInQuery)) *CustomerQuery {
	query := &CheckInQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withReserves; query != nil {
		...
		query.Where(predicate.ReserveRoom(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.ReservesColumn, fks...))
		}))
	...
	if query := cq.withFixs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Customer)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.FixRoom(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.FixsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := cq.withCheckins; query != nil {
		...
		query.Where(predicate.CheckIn(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.CheckinsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
