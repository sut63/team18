...
func (tx *Tx) init() {
	...
	tx.CounterStaff = NewCounterStaffClient(tx.config)
	tx.Customer = NewCustomerClient(tx.config)
	tx.DataRoom = NewDataRoomClient(tx.config)
	tx.FixRoom = NewFixRoomClient(tx.config)
	tx.Furniture = NewFurnitureClient(tx.config)
	tx.FurnitureDetail = NewFurnitureDetailClient(tx.config)
	tx.FurnitureType = NewFurnitureTypeClient(tx.config)
...
// applies a query, for example: CheckIn.QueryXXX(), the query will be executed
