...
func (sq *StatusQuery) QueryCheckouts() *CheckoutQuery {
	query := &CheckoutQuery{config: sq.config}
...
func (sq *StatusQuery) WithCheckouts(opts ...func(*CheckoutQuery)) *StatusQuery {
	query := &CheckoutQuery{config: sq.config}
...
//
//	client.Status.Query().
//		GroupBy(status.FieldDescription).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Status.Query().
//		Select(status.FieldDescription).
//		Scan(ctx, &v)
//
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	...
	if query := sq.withCheckouts; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(status.CheckoutsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.status_checkouts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "status_checkouts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_checkouts" returned %v for node %v`, *fk, n.ID)
			}
