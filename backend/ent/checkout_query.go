...
// QueryStatuss chains the current query on the statuss edge.
func (cq *CheckoutQuery) QueryStatuss() *StatusQuery {
	query := &StatusQuery{config: cq.config}
...
func (cq *CheckoutQuery) QueryCounterstaffs() *CounterStaffQuery {
	query := &CounterStaffQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CheckoutQuery) QueryCheckins() *CheckInQuery {
	query := &CheckInQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithStatuss tells the query-builder to eager-loads the nodes that are connected to
// the "statuss" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CheckoutQuery) WithStatuss(opts ...func(*StatusQuery)) *CheckoutQuery {
	query := &StatusQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withStatuss = query
	return cq
}
...
func (cq *CheckoutQuery) WithCheckins(opts ...func(*CheckInQuery)) *CheckoutQuery {
	query := &CheckInQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCheckins = query
	return cq
}
...
func (cq *CheckoutQuery) sqlAll(ctx context.Context) ([]*Checkout, error) {
	...
	if query := cq.withStatuss; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Checkout)
		for i := range nodes {
			if fk := nodes[i].status_checkouts; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_checkouts" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withCounterstaffs; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Checkout)
		for i := range nodes {
	...
	if query := cq.withCheckins; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Checkout)
		for i := range nodes {
			if fk := nodes[i].check_in_checkouts; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(checkin.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "check_in_checkouts" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Checkins = n
			}
		}
	}
