...
func (pq *PromotionQuery) QueryReserves() *ReserveRoomQuery {
	query := &ReserveRoomQuery{config: pq.config}
...
func (pq *PromotionQuery) QueryDatarooms() *DataRoomQuery {
	query := &DataRoomQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PromotionQuery) WithReserves(opts ...func(*ReserveRoomQuery)) *PromotionQuery {
	query := &ReserveRoomQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withReserves = query
	return pq
}
...
func (pq *PromotionQuery) WithDatarooms(opts ...func(*DataRoomQuery)) *PromotionQuery {
	query := &DataRoomQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withDatarooms = query
	return pq
}
...
func (pq *PromotionQuery) sqlAll(ctx context.Context) ([]*Promotion, error) {
	...
	if query := pq.withReserves; query != nil {
		...
		query.Where(predicate.ReserveRoom(func(s *sql.Selector) {
			s.Where(sql.InValues(promotion.ReservesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.promotion_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "promotion_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "promotion_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Reserves = append(node.Edges.Reserves, n)
		}
	}

	if query := pq.withDatarooms; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Promotion)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.DataRoom(func(s *sql.Selector) {
			s.Where(sql.InValues(promotion.DataroomsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
