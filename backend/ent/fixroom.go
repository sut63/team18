...
func (fr *FixRoom) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		...
		fr.PhoneNumber = value.String
	}
	if value, ok := values[2].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field facebook", values[2])
	} else if value.Valid {
		fr.Facebook = value.String
	}
	values = values[3:]
...
func (fr *FixRoom) String() string {
	...
	builder.WriteString(fr.PhoneNumber)
	builder.WriteString(", facebook=")
	builder.WriteString(fr.Facebook)
	builder.WriteByte(')')
